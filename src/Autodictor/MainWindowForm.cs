...
namespace MainExample
{
    ...
    {
        private readonly Func<СтатическоеСообщение, КарточкаСтатическогоЗвуковогоСообщенияForm>
            _staticSoundCardFormFactory;
        ...
        public GetSheduleDispatcherControl
            GetSheduleDispatcherControl { get; set; } //сервис получения данных от диспетчера
            ...
            Func<BaseGetDataBehavior, GetCisRegSh> getCisRegShFactory)
        {
            ...
            {
                путь0ToolStripMenuItem, путь1ToolStripMenuItem, путь2ToolStripMenuItem, путь3ToolStripMenuItem,
                путь4ToolStripMenuItem, путь5ToolStripMenuItem, путь6ToolStripMenuItem, путь7ToolStripMenuItem,
                путь8ToolStripMenuItem, путь9ToolStripMenuItem, путь10ToolStripMenuItem, путь11ToolStripMenuItem,
                путь12ToolStripMenuItem, путь13ToolStripMenuItem, путь14ToolStripMenuItem, путь15ToolStripMenuItem,
                путь16ToolStripMenuItem, путь17ToolStripMenuItem, путь18ToolStripMenuItem, путь19ToolStripMenuItem,
                путь20ToolStripMenuItem, путь21ToolStripMenuItem, путь22ToolStripMenuItem, путь23ToolStripMenuItem,
                путь24ToolStripMenuItem, путь25ToolStripMenuItem
            };
            ...
            Program.ЗаписьЛога("Системное сообщение", "Программный комплекс включен",
                _authentificationService.CurrentUser);
        }
            ...
            foreach (var beh in Binding2GetDataBehaviors)
            {
                ...
                switch (beh.GetDeviceName)
                {
                    ...
                    case "HttpDispatcher":
                        ...
                        GetSheduleDispatcherControl =
                            new GetSheduleDispatcherControl(beh.BaseGetDataBehavior, SoundRecords);
        ...
        private void TemplateChangeRxEventHandler(TemplateChangeValue templateChangeValue)
        {
            ...
            switch (templateChangeValue.SoundMessage.MessageType)
            {
                case MessageType.Динамическое:
                    var soundRecordKeyValuePair =
                        SoundRecords.FirstOrDefault(rec => rec.Value.Id == templateChangeValue.SoundMessage.RootId);
                    var record = soundRecordKeyValuePair.Value;
                    var template = record.ActionTrainDynamiсList.FirstOrDefault(actDyn =>
                        actDyn.Id == templateChangeValue.SoundMessage.ParentId);
                    if (template == null) return;
                    switch (templateChangeValue.StatusPlaying)
                    {
                        case StatusPlaying.Start:
                            template.SoundRecordStatus =
                                (template.SoundRecordStatus == SoundRecordStatus.ДобавленВОчередьРучное)
                                    ? SoundRecordStatus.ВоспроизведениеРучное
                                    : SoundRecordStatus.ВоспроизведениеАвтомат;
                            break;
                        case StatusPlaying.Stop:
                ...
                case MessageType.ДинамическоеАварийное:
                    soundRecordKeyValuePair =
                        SoundRecords.FirstOrDefault(rec => rec.Value.Id == templateChangeValue.SoundMessage.RootId);
                    record = soundRecordKeyValuePair.Value;
                    template = record.EmergencyTrainDynamiсList.FirstOrDefault(actDyn =>
                        actDyn.Id == templateChangeValue.SoundMessage.ParentId);
                    if (template == null) return;
                    switch (templateChangeValue.StatusPlaying)
                    {
                ...
                case MessageType.ДинамическоеТехническое:
                    var soundRecordTech =
                        TechnicalMessageForm.SoundRecords.FirstOrDefault(rec =>
                            rec.Id == templateChangeValue.SoundMessage.RootId);
                    if (soundRecordTech.Id > 0) // если найден (в дефолтном значении Id = 0)
                    {
                        template = soundRecordTech.ActionTrainDynamiсList.FirstOrDefault(actDyn =>
                            actDyn.Id == templateChangeValue.SoundMessage.ParentId);
                        if (template == null) return;
                        switch (templateChangeValue.StatusPlaying)
                        {
        ...
        private void ИнициализироватьВсеТабло()
        {
            for (var i = 0; i < SoundRecords.Count; i++)
            {
                ...
                if (!string.IsNullOrEmpty(данные.НомерПути))
                {
                    ...
                    данные.СостояниеОтображения = (данные.НомерПути == string.Empty || данные.НомерПути == "0")
                        ? TableRecordStatus.Очистка
        ...
        public void СозданиеРасписанияЖдТранспорта()
        {
            ...
            SoundRecordChanges = _soundRecChangesService.Get(p => (p.TimeStamp.Date == currentDay) ||
                                                                  ((p.TimeStamp.Date == currentDay.AddDays(-1)) &&
                                                                   (p.Rec.Время.Date == currentDay)))
        ...
        private void СозданиеЗвуковыхФайловРасписанияЖдТранспорта(IList<TrainTableRec> trainTableRecords, DateTime день,
            Func<int, bool> ограничениеВремениПоЧасам, ref int id)
        {
            ...
            for (var index = 0; index < trainTableRecords.Count; index++)
            {
                ...
                if (config.Active == false &&
                    Program.Настройки.РазрешениеДобавленияЗаблокированныхПоездовВСписок == false)
                    continue;

                if (!pipelineService.CheckTrainActuality(config, день, ограничениеВремениПоЧасам,
                    РаботаПоНомеруДняНедели))
                    continue;

                var newId = id++;
                SoundRecord record =
                    Mapper.MapTrainTableRecord2SoundRecord(config, _soundReсordWorkerService, день, newId);
        ...
        private void КорректировкаЗаписейПоИзменениям()
        {
            ...
            var filtredOnMaxDate = SoundRecordChanges
                .GroupBy(gr => new {gr.ScheduleId, gr.Rec.НомерПоезда, gr.Rec.Время.Date})
        ...
        private void ПрименениеЗагруженныхИзменений(SoundRecord rec, SoundRecord newRec, string key)
        {
            ...
            rec.Emergency = newRec.Emergency;
            ...
            listView1.InvokeIfNeeded(() =>
            {
                ...
                for (int i = 0; i < SoundRecords.Count; i++)
                {
                    ...
                    if ((Данные.Value.БитыАктивностиПолей & 0x04) != 0x00)
                        ВремяПрибытия = Данные.Value.ВремяПрибытия.ToString("HH:mm");
                    if ((Данные.Value.БитыАктивностиПолей & 0x10) != 0x00)
                        ВремяОтправления = Данные.Value.ВремяОтправления.ToString("HH:mm");


                    ListViewItem lvi1 = new ListViewItem(new string[]
                    {
                        Данные.Value.Время.ToString("yy.MM.dd  HH:mm:ss"),
                        Данные.Value.НомерПоезда.Replace(':', ' '),
                        Данные.Value.НомерПути.ToString(),
                        Данные.Value.НазваниеПоезда,
                        ВремяПрибытия,
                        ВремяОтправления,
                        Данные.Value.Route?.ToString() ?? String.Empty,
        ...
        private void ОбновитьСписокЗвуковыхСообщенийВТаблицеСтатическихСообщений()
        {
            ...
            foreach (var Данные in СтатическиеЗвуковыеСообщения)
            {
                ...
                {
                    if (lVСтатическиеСообщения.Items[НомерСтроки].SubItems[0].Text !=
                        Данные.Value.Время.ToString("yy.MM.dd  HH:mm:ss"))
                        lVСтатическиеСообщения.Items[НомерСтроки].SubItems[0].Text =
                            Данные.Value.Время.ToString("yy.MM.dd  HH:mm:ss");
                        ...
                        if (SoundRecords.Keys.Contains(Key) == true)
                        {
                            ...
                            switch (данные.СостояниеКарточки)
                            {
                                ...
                                case 0: // Выключен или не актуален
                                    if (lv.Items[item].ForeColor != ((foreColor == Color.Black)
                                            ? Program.Настройки.НастройкиЦветов[0]
                                            : foreColor))
                                        lv.Items[item].ForeColor = ((foreColor == Color.Black)
                                            ? Program.Настройки.НастройкиЦветов[0]
                                            : foreColor);
                                ...
                                case 1: // Отсутствую шаблоны оповещения
                                    if (lv.Items[item].ForeColor != ((foreColor == Color.Black)
                                            ? Program.Настройки.НастройкиЦветов[2]
                                            : foreColor))
                                        lv.Items[item].ForeColor = ((foreColor == Color.Black)
                                            ? Program.Настройки.НастройкиЦветов[2]
                                            : foreColor);
                                ...
                                case 2: // Время не подошло (за 30 минут)
                                    if (lv.Items[item].ForeColor != ((foreColor == Color.Black)
                                            ? Program.Настройки.НастройкиЦветов[4]
                                            : foreColor))
                                        lv.Items[item].ForeColor = ((foreColor == Color.Black)
                                            ? Program.Настройки.НастройкиЦветов[4]
                                            : foreColor);
                                ...
                                case 3: // Не установлен путь
                                    if (lv.Items[item].ForeColor != ((foreColor == Color.Black)
                                            ? Program.Настройки.НастройкиЦветов[6]
                                            : foreColor))
                                        lv.Items[item].ForeColor = ((foreColor == Color.Black)
                                            ? Program.Настройки.НастройкиЦветов[6]
                                            : foreColor);
                                ...
                                case 4: // Не полностью включены все галочки
                                    if (lv.Items[item].ForeColor != ((foreColor == Color.Black)
                                            ? Program.Настройки.НастройкиЦветов[8]
                                            : foreColor))
                                        lv.Items[item].ForeColor = ((foreColor == Color.Black)
                                            ? Program.Настройки.НастройкиЦветов[8]
                                            : foreColor);
                                ...
                                case 5: // Полностью включены все галочки
                                    if (lv.Items[item].ForeColor != ((foreColor == Color.Black)
                                            ? Program.Настройки.НастройкиЦветов[10]
                                            : foreColor))
                                        lv.Items[item].ForeColor = ((foreColor == Color.Black)
                                            ? Program.Настройки.НастройкиЦветов[10]
                                            : foreColor);
                                ...
                                case 36: // Нештатная ситуация "Отпр по готов"
                                    if (lv.Items[item].ForeColor != ((foreColor == Color.Black)
                                            ? Program.Настройки.НастройкиЦветов[12]
                                            : foreColor))
                                        lv.Items[item].ForeColor = ((foreColor == Color.Black)
                                            ? Program.Настройки.НастройкиЦветов[12]
                                            : foreColor);
                                ...
                                case 7: // Ручной режим за 30 мин до самого ранего события или если не выставленн ПУТЬ
                                    if (lv.Items[item].ForeColor != ((foreColor == Color.Black)
                                            ? Program.Настройки.НастройкиЦветов[14]
                                            : foreColor))
                                        lv.Items[item].ForeColor = ((foreColor == Color.Black)
                                            ? Program.Настройки.НастройкиЦветов[14]
                                            : foreColor);
                                ...
                                case 8: // Ручной режим
                                    if (lv.Items[item].ForeColor !=
                                        ((foreColor == Color.Black) ? Color.White : foreColor))
                                        lv.Items[item].ForeColor =
                                            ((foreColor == Color.Black) ? Color.White : foreColor);
                                    if (lv.Items[item].BackColor != Program.Настройки.НастройкиЦветов[15])
                                        lv.Items[item].BackColor = Program.Настройки.НастройкиЦветов[15];
                                    if ((Math.Abs(lv.Items[item].Font.Size - font.Size) > 0.2) ||
                                        (font.Name != lv.Items[item].Font.Name))
                                    {
                                        lv.Items[item].Font = font;
                                        SetHeight(listView1, (int) (font.Size * 2));
                            ...
                            if (lv.Name == "listView1")
                            {
                                ...
                                var routeStr = (данные.Route?.ToString() ?? string.Empty) + нумерацияПоезда;
        ...
        private void ОпределитьКомпозициюДляЗапуска()
        {
            ...
            for (int i = 0; i < СтатическиеЗвуковыеСообщения.Count(); i++)
            {
                ...
                {
                    if (QueueSound.FindItem(Сообщение.ID, null) == null
                    ...
                    if (Сообщение.Активность == true)
                        foreach (var Sound in StaticSoundForm.StaticSoundRecords)
                        {
                            if (Sound.Name == Сообщение.НазваниеКомпозиции)
                            {
                                ...
                                {
                                    Program.ЗаписьЛога(
                                        "Автоматическое воспроизведение статического звукового сообщения",
                                        Сообщение.НазваниеКомпозиции, _authentificationService.CurrentUser);
                ...
                if (DateTime.Now > Сообщение.Время.AddMinutes(-30) &&
                    !(Сообщение.СостояниеВоспроизведения == SoundRecordStatus.Воспроизведена &&
                      DateTime.Now > Сообщение.Время.AddSeconds(ВремяЗадержкиВоспроизведенныхСобытий))
                ...
                while (true)
                {
                    if (данные.Активность)
                    {
                        ...
                        if (данные.Emergency != Emergency.None)
                        {
                            ...
                            if ((данные.СостояниеКарточки != 6) && (данные.Emergency == Emergency.Cancel))
                            {
                                данные.ОписаниеСостоянияКарточки = "Поезд отменен";
                                данные.СостояниеКарточки = 6;
                                внесеныИзменения = true;
                            }
                            else if (данные.СостояниеКарточки != 16 && (данные.Emergency == Emergency.DelayedArrival))
                            {
                                данные.ОписаниеСостоянияКарточки = "Задержка прибытия поезда";
                                данные.СостояниеКарточки = 16;
                                внесеныИзменения = true;
                            }
                            else if (данные.СостояниеКарточки != 26 && (данные.Emergency == Emergency.DelayedDeparture))
                            {
                                данные.ОписаниеСостоянияКарточки = "Задержка отправления поезда";
                                данные.СостояниеКарточки = 26;
                                внесеныИзменения = true;
                            }
                            else if (данные.СостояниеКарточки != 36 &&
                                     (данные.Emergency == Emergency.DispatchOnReadiness))
                            {
                            ...
                            if (данные.Автомат)
                            {
                                ...
                                for (int j = 0; j < данные.EmergencyTrainDynamiсList.Count; j++)
                                {
                                    ...
                                    {
                                        if (emergencyAction.SoundRecordStatus ==
                                            SoundRecordStatus.ДобавленВОчередьРучное ||
                                            emergencyAction.SoundRecordStatus ==
                                            SoundRecordStatus.ВоспроизведениеРучное)
                                        {
                                            continue;
                                        }
                                        ...
                                        var activationTime =
                                            _soundReсordWorkerService.CalcTimeWithShift(ref данные, emergencyAction);
                                        ...
                                        {
                                            if (QueueSound.FindItem(данные.Id, emergencyAction.Id) == null
                                            ) //Если нету элемента в очереди сообщений, то запись уже воспроизведенна.
                                            {
                                                if (emergencyAction.SoundRecordStatus !=
                                        ...
                                        else if (emergencyAction.SoundRecordStatus ==
                                                 SoundRecordStatus.ОжиданиеВоспроизведения)
                                        {
                                            // СРАБОТКА------------------------------------------------------------
                                            if ((текущееВремя.Hour == activationTime.Hour) &&
                                                (текущееВремя.Minute == activationTime.Minute) &&
                                                (текущееВремя.Second == activationTime.Second))
                                            {
                                                emergencyAction.SoundRecordStatus =
                                                ...
                                                if (РазрешениеРаботы)
                                                {
                                                    ВоспроизвестиШаблонОповещения_New(
                                                        "Автоматическое воспроизведение сообщения о внештатной ситуации",
                                                        данные, emergencyAction, MessageType.ДинамическоеАварийное);
                                                }
                                            }
                                        }

                                        //Добавление НЕШТАТНОГО события ===================================================================
                                        if (DateTime.Now > activationTime.AddMinutes(-30) &&
                                            !(emergencyAction.SoundRecordStatus == SoundRecordStatus.Воспроизведена &&
                                              DateTime.Now >
                                              activationTime.AddSeconds(ВремяЗадержкиВоспроизведенныхСобытий))
                                        ) //убрать через 5 мин. после воспроизведения
                                        {
                                            StateTask состояниеСтроки = StateTask.Disabled;
                                            ...
                                            TaskSound taskSound = new TaskSound
                                            {
                                                ...
                                                StateTask = состояниеСтроки,
                                                Описание = данные.НомерПоезда + " " + данные.НазваниеПоезда + ": " +
                        ...
                        for (int j = 0; j < данные.ActionTrainDynamiсList.Count; j++)
                        {
                            ...
                            if (!данные.Автомат)
                            {
                                if (actionTrainDyn.ActionTrain.Name.StartsWith("@") &&
                            ...
                            var activationTime =
                                _soundReсordWorkerService.CalcTimeWithShift(ref данные, actionTrainDyn);
                            if (j == 0)
                            {
                        ...
                        if (DateTime.Now <
                            самоеРаннееВремя.AddMinutes(Program.Настройки.ОповещениеСамогоРаннегоВремениШаблона))
                        {
                            if (!данные.Автомат)
                            {
                        ...
                        if (данные.НомерПути == "")
                        {
                            ...
                            else if (данные.СостояниеКарточки != 3)
                            {
                        ...
                        for (int j = 0; j < данные.ActionTrainDynamiсList.Count; j++)
                        {
                            ...
                            var activationTime =
                                _soundReсordWorkerService.CalcTimeWithShift(ref данные, actionTrainDyn); //времяСобытия
                            if (actionTrainDyn.Activity == true)
                            {
                                ...
                                if (DateTime.Now < activationTime)
                                {
                                    ...
                                    {
                                        if (actionTrainDyn.SoundRecordStatus !=
                                            SoundRecordStatus.ОжиданиеВоспроизведения
                                        ) //TODO: При ручном воспр. стаатус сразу меняется на ОжиданиеВоспроизведения
                                        {
                                            actionTrainDyn.SoundRecordStatus =
                                                SoundRecordStatus.ОжиданиеВоспроизведения;
                                            внесеныИзменения = true;
                                        }
                                    }
                                ...
                                else if (actionTrainDyn.SoundRecordStatus == SoundRecordStatus.ОжиданиеВоспроизведения)
                                {
                                        ...
                                        (текущееВремя.Second >= activationTime.Second))
                                    {
                                        ...
                                        if (РазрешениеРаботы == true)
                                            ВоспроизвестиШаблонОповещения_New(
                                                "Автоматическое воспроизведение расписания", данные, actionTrainDyn,
                                                MessageType.Динамическое);
                                    }
                                }
                                ...
                                if (DateTime.Now > activationTime.AddMinutes(-30) &&
                                    !(actionTrainDyn.SoundRecordStatus == SoundRecordStatus.Воспроизведена &&
                                      DateTime.Now > activationTime.AddSeconds(ВремяЗадержкиВоспроизведенныхСобытий)))
                                {
        ...
        private void ОпределитьИнформациюДляОтображенияНаТабло()
        {
            ...
            if (_tickCounter++ > 50)
            {
                ...
                if (Binding2GeneralScheduleBehaviors != null && Binding2GeneralScheduleBehaviors.Any())
                {
                    ...
                    {
                        var trainRecs = _trainRecService.GetAll();
                        if (trainRecs != null)
                        {
                            foreach (var beh in binding2Shedule)
                            {
                                ...
                                table.ForEach(t =>
                                {
                                    ...
                                    t.Message =
                                        $"ПОЕЗД:{t.NumberOfTrain}, ПУТЬ:{t.PathNumber}, СОБЫТИЕ:{t.EventOld}, СТАНЦИИ:{t.Stations}, ВРЕМЯ:{t.Time.ToShortTimeString()}";
                                });

                                var inData = new UniversalInputType {TableData = table};
                                beh.InitializePagingBuffer(inData, defaultType, beh.CheckContrains,
                                    beh.GetCountDataTake());
                            }
                        }
                    }
                    ...
                    {
                        if (TrainTableOperativeForm.TrainTableRecords != null)
                        {
                            foreach (var beh in binding2OperativeShedule)
                            {
                                var table = TrainTableOperativeForm.TrainTableRecords
                                    .Select(Mapper.MapTrainTableRecord2UniversalInputType)
                                    .ToList();
                        ...
                        if (SoundRecords != null && SoundRecords.Any())
                        {
                            foreach (var beh in binding2MainWindow)
                            {
                                var table = SoundRecords
                                    .Select(t =>
                                        Mapper.MapSoundRecord2UniveralInputType(t.Value,
                                            beh.GetDeviceSetting.PathPermission, false))
                                    .ToList();
                ...
                if (Binding2ChangesBehaviors != null && Binding2ChangesBehaviors.Any())
                {
                    foreach (var beh in Binding2ChangesBehaviors)
                    {
                        ...
                        {
                            var uit = Mapper.MapSoundRecord2UniveralInputType(change.Rec,
                                beh.GetDeviceSetting.PathPermission, false);
                            ...
                            var uitNew = Mapper.MapSoundRecord2UniveralInputType(change.NewRec,
                                beh.GetDeviceSetting.PathPermission, false);
            ...
            for (var i = 0; i < SoundRecords.Count; i++)
            {
                try
                {
                    ...
                    if (_checked && (данные.ТипСообщения == SoundRecordType.ДвижениеПоезда))
                    {
                        ...
                        var номераПутей = _trainRecService.GetPathways().ToList();
                        ...
                        if (номерПути > 0 || (номерПути == 0 && номерПутиOld > 0))
                        {
                                ...
                                (данные.БитыАктивностиПолей & 0x14) == 0x14)
                            {
                                ...
                                if ((данные.Emergency == Emergency.None)
                                    && (данныеOld.Emergency != Emergency.None)
        ...
        private void ОбработкаЗвуковогоПотка()
        {
            ...
            SoundPlayerStatus
                status = Program.AutodictorModel.SoundPlayer.GetPlayerStatus(); //PlayerDirectX.GetPlayerStatus();
            switch (status)
            {
        ...
        private void CheckAutoApdate()
        {
            ...
            if ((DateTime.Now.Hour == hourAutoApdate) && (DateTime.Now.Minute == minuteAutoApdate) &&
                (DateTime.Now.Second == secondAutoApdate))
            {
                btnОбновитьСписок_Click(null, null);
            }
        }
        ...
        private void СобытиеНачалоПроигрыванияОчередиЗвуковыхСообщений()
        {
            ...
            {
                var soundChUit = new UniversalInputType
                {
                    SoundChanels = Program.Настройки.КаналыДальнегоСлед.ToList(),
        ...
        {
            if (data.СостояниеОтображения == TableRecordStatus.Выключена ||
                data.СостояниеОтображения == TableRecordStatus.ОжиданиеОтображения)
                return;
            ...
            var devicesId = data.НазванияТабло.Select(s => new string(s.TakeWhile(c => c != ':').ToArray()))
            ...
            foreach (var devId in devicesId)
            {
                ...
                {
                    var inData =
                        Mapper.MapSoundRecord2UniveralInputType(data, beh.GetDeviceSetting.PathPermission, true);
                    var uitPreprocessingService = PreprocessingOutputFactory.CreateUitPreprocessingOutputService();
                    uitPreprocessingService.StartPreprocessing(inData);
                    inData.Message =
                        $"ПОЕЗД:{inData.NumberOfTrain}, ПУТЬ:{inData.PathNumber}, СОБЫТИЕ:{inData.EventOld}, СТАНЦИИ:{inData.Stations}, ВРЕМЯ:{inData.Time.ToShortTimeString()}";

                    var numberOfTrain =
                        (string.IsNullOrEmpty(data.НомерПоезда2) || string.IsNullOrWhiteSpace(data.НомерПоезда2))
                            ? data.НомерПоезда
                            : (data.НомерПоезда + "/" + data.НомерПоезда2);
            ...
            try
            {
                ...
                foreach (int item in sic)
                {
                    if (item <= СтатическиеЗвуковыеСообщения.Count)
                    {
                        ...
                        {
                            СтатическоеСообщение Данные = СтатическиеЗвуковыеСообщения[Key];
                            КарточкаСтатическогоЗвуковогоСообщенияForm окноСообщенияForm =
                                _staticSoundCardFormFactory(Данные);
                            if (окноСообщенияForm.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                            {
                                ...
                                if (Key == Key2)
                                {
                                    ...
                                    this.lVСтатическиеСообщения.Items[item].SubItems[1].Text =
                                        Данные.НазваниеКомпозиции;
                                }
                                else
                                {
        ...
        public SoundRecord ПрименитьИзмененияSoundRecord(SoundRecord данные, SoundRecord старыеДанные, string key,
            string keyOld, ListView listView)
        {
            ...
            listView.InvokeIfNeeded(() =>
            {
                ...
                switch (listView.Name)
                {
                    case "listView1":
                        ...
                        if (listView.Items[item].SubItems[7].Text != данные.Дополнение) //Изменение ДОПОЛНЕНИЯ
                            listView.Items[item].SubItems[7].Text = данные.ИспользоватьДополнение["звук"]
                                ? данные.Дополнение
                                : String.Empty;
                        break;
                    ...
                    case "lVОтправление":
                        ...
                        if (listView.Items[item].SubItems[5].Text != данные.НазваниеПоезда)
                            listView.Items[item].SubItems[5].Text = данные.ИспользоватьДополнение["звук"]
                                ? данные.Дополнение
                                : String.Empty;
                        break;
                    ...
                    case "lVТранзит":
                        ...
                        if (listView.Items[item].SubItems[6].Text != данные.НазваниеПоезда)
                            listView.Items[item].SubItems[6].Text = данные.ИспользоватьДополнение["звук"]
                                ? данные.Дополнение
                                : String.Empty;
                        break;
                }
                ...
                if (SoundRecords.ContainsKey(keyOld) == false
                ) // поменяли время приб. или отпр. т.е. изменили ключ записи. Т.е. удалили запись под старым ключем.
                {
        ...
        private void lVПрибытие_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Right)
            {
                ...
                if ((list.Name == "lVПрибытие") || (list.Name == "lVТранзит") || (list.Name == "lVОтправление") ||
                    (list.Name == "listView1"))
                {
                            ...
                            foreach (int item in sic)
                            {
                                if (item <= SoundRecords.Count)
                                {
                                    ...
                                    if (SoundRecords.Keys.Contains(Key) == true)
                                    {
                                        ...
                                        var paths = _trainRecService.GetPathways().Select(p => p.Name).ToList();
                                        ...
                                        ToolStripMenuItem[] СписокНумерацииВагонов = new ToolStripMenuItem[]
                                        {
                                            отсутсвуетToolStripMenuItem, сГоловыСоставаToolStripMenuItem,
                                        ...
                                        ToolStripMenuItem[] СписокКоличестваПовторов = new ToolStripMenuItem[]
                                        {
                                            null, повтор1ToolStripMenuItem, повтор2ToolStripMenuItem,
                                        ...
                                        var вариантыОтображенияПути =
                                            Табло_отображениеПутиToolStripMenuItem.DropDownItems;
                        ...
                        try
                        {
                            ...
                            foreach (int item in sic)
                            {
                                if (item <= СтатическиеЗвуковыеСообщения.Count)
                                {
                                    ...
                                    {
                                        СтатическоеСообщение Данные = СтатическиеЗвуковыеСообщения[Key];
                                        включитьToolStripMenuItem.Text =
                                            Данные.Активность == true ? "Отключить" : "Включить";
                                    }
                                }
                            }
                        }
        ...
        public static void ВоспроизвестиШаблонОповещения_New(string названиеСообщения, SoundRecord rec,
            ActionTrainDynamic actionTrainDynamic, MessageType messageType)
        {
            if (!rec.ВыводЗвука)
                return;
            ...
            string[] файлыМинут = new string[]
            {
                "00 минут", "01 минута", "02 минуты", "03 минуты", "04 минуты", "05 минут", "06 минут", "07 минут",
                "08 минут",
                "09 минут", "10 минут", "11 минут", "12 минут", "13 минут", "14 минут", "15 минут", "16 минут",
                "17 минут",
                "18 минут", "19 минут", "20 минут", "21 минута", "22 минуты", "23 минуты", "24 минуты", "25 минут",
                "26 минут",
                "27 минут", "28 минут", "29 минут", "30 минут", "31 минута", "32 минуты", "33 минуты", "34 минуты",
                "35 минут",
                "36 минут", "37 минут", "38 минут", "39 минут", "40 минут", "41 минута", "42 минуты", "43 минуты",
                "44 минуты",
                "45 минут", "46 минут", "47 минут", "48 минут", "49 минут", "50 минут", "51 минута", "52 минуты",
                "53 минуты",
                "54 минуты", "55 минут", "56 минут", "57 минут", "58 минут", "59 минут"
            };


            string[] файлыЧасовПрефиксВ = new string[]
            {
                "В 00 часов", "В 01 час", "В 02 часа", "В 03 часа", "В 04 часа", "В 05 часов", "В 06 часов",
                "В 07 часов",
                "В 08 часов", "В 09 часов", "В 10 часов", "В 11 часов", "В 12 часов", "В 13 часов", "В 14 часов",
                "В 15 часов",
                "В 16 часов", "В 17 часов", "В 18 часов", "В 19 часов", "В 20 часов", "В 21 час", "В 22 часа",
                "В 23 часа"
            };

            string[] файлыЧасов = new string[]
            {
                "00 часов", "01 час", "02 часа", "03 часа", "04 часа", "05 часов", "06 часов", "07 часов",
            ...
            foreach (var lang in actionTrain.Langs)
            {
                ...
                var notificationLang =
                    (NotificationLanguage) Enum.Parse(typeof(NotificationLanguage),
                        lang.Name); //TODO: NotificationLanguage заменить на string
                foreach (var item in templateItems)
                {
                    ...
                    switch (template)
                    {
                        ...
                        case "НОМЕР ПОЕЗДА":
                            txt = rec.НомерПоезда;
                            var fileNames = numeric2ListStringConverter.Convert(txt)
                                ?.Where(f => f != "0" && f != "0" + eof).ToList();
                        ...
                        case "НОМЕР ПОЕЗДА ТРАНЗИТ ОТПР":
                            if (!string.IsNullOrEmpty(rec.НомерПоезда2))
                            {
                                ...
                                fileNames = numeric2ListStringConverter.Convert(txt)
                                    ?.Where(f => f != "0" && f != "0" + eof).ToList();
                                if (fileNames != null && fileNames.Any())
                                {
                                    foreach (var fileName in fileNames)
                        ...
                        case "ВРЕМЯ ПРИБЫТИЯ":
                            воспроизводимоеСообщение.ИмяВоспроизводимогоФайла =
                                файлыЧасовПрефиксВ[rec.ВремяПрибытия.Hour];
                            воспроизводимыеСообщения.Add(воспроизводимоеСообщение);
                            воспроизводимыеСообщения.Add(new ВоспроизводимоеСообщение
                            {
                        ...
                        case "ВРЕМЯ СТОЯНКИ":
                            if (rec.ВремяСтоянки.HasValue)
                            {
                                ...
                                {
                                    воспроизводимоеСообщение.ИмяВоспроизводимогоФайла =
                                        файлыЧасов[rec.ВремяСтоянки.Value.Hours];
                                    воспроизводимыеСообщения.Add(воспроизводимоеСообщение);
                                }
                                воспроизводимыеСообщения.Add(new ВоспроизводимоеСообщение
                                {
                                ...
                                logMessage += "Стоянка: " + rec.ВремяСтоянки.Value.Hours.ToString("D2") + ":" +
                                              rec.ВремяСтоянки.Value.Minutes.ToString("D2") + " минут" + " ";
                            }
                            else if (rec.БитыАктивностиПолей == 31
                            ) //У трнзита нет времени стоянки, занчит стоит галочка "будет измененно"
                            {
                        ...
                        case "ВРЕМЯ ОТПРАВЛЕНИЯ":
                            воспроизводимоеСообщение.ИмяВоспроизводимогоФайла =
                                файлыЧасовПрефиксВ[rec.ВремяОтправления.Hour];
                            воспроизводимыеСообщения.Add(воспроизводимоеСообщение);
                            воспроизводимыеСообщения.Add(new ВоспроизводимоеСообщение
                            {
                        ...
                        case "ВРЕМЯ ЗАДЕРЖКИ":
                            if (rec.ВремяЗадержки != null)
                            {
                                ...
                                {
                                    воспроизводимоеСообщение.ИмяВоспроизводимогоФайла =
                                        файлыЧасовПрефиксВ[rec.ВремяЗадержки.Value.Hour];
                                    воспроизводимыеСообщения.Add(воспроизводимоеСообщение);
                                }
                                воспроизводимыеСообщения.Add(new ВоспроизводимоеСообщение
                                {
                        ...
                        case "ОЖИДАЕМОЕ ВРЕМЯ":
                            воспроизводимоеСообщение.ИмяВоспроизводимогоФайла =
                                файлыЧасовПрефиксВ[rec.ОжидаемоеВремя.Hour];
                            воспроизводимыеСообщения.Add(воспроизводимоеСообщение);
                            воспроизводимыеСообщения.Add(new ВоспроизводимоеСообщение
                            {
                        ...
                        case "ВРЕМЯ СЛЕДОВАНИЯ":
                            ...
                            воспроизводимоеСообщение.ИмяВоспроизводимогоФайла =
                                файлыЧасовПрефиксВ[rec.ВремяСледования.Value.Hour];
                            воспроизводимыеСообщения.Add(воспроизводимоеСообщение);
                            воспроизводимыеСообщения.Add(new ВоспроизводимоеСообщение
                            {
                            ...
                            if (rec.ТипПоезда.CategoryTrain == CategoryTrain.Suburb)
                            {
                                ...
                                switch (rec.Route.RouteType)
                                {
                                    case RouteType.WithStopsAt:
                                        ...
                                        logMessage = routeStations.Aggregate(logMessage,
                                            (current, станция) => current + (станция + " "));
                                        break;
                    ...
                    if (item <= СтатическиеЗвуковыеСообщения.Count)
                    {
                        ...
                        if (СтатическиеЗвуковыеСообщения.Keys.Contains(Key) == true)
                        {
                            ...
                            foreach (var Sound in StaticSoundForm.StaticSoundRecords)
                            {
                                ...
                                {
                                    Program.ЗаписьЛога("Действие оператора",
                                        "ВоспроизведениеАвтомат статического звукового сообщения: " + Sound.Name,
                                        _authentificationService.CurrentUser);
                                    var воспроизводимоеСообщение = new ВоспроизводимоеСообщение
                                    {
                                        ParentId = null,
                ...
                foreach (int item in sic)
                {
                    if (item <= СтатическиеЗвуковыеСообщения.Count)
                    {
                        ...
                        if (СтатическиеЗвуковыеСообщения.Keys.Contains(Key) == true)
                        {
                            ...
                            Program.ЗаписьЛога("Действие оператора",
                                (Данные.Активность ? "Включение " : "Отключение ") + "звукового сообщения: \"" +
                                Данные.НазваниеКомпозиции + "\" (" + Данные.Время.ToString("HH:mm") + ")",
                                _authentificationService.CurrentUser);
                            СтатическиеЗвуковыеСообщения[Key] = Данные;
                        }
                    }
                ...
                if (SoundRecords.Keys.Contains(КлючВыбранныйМеню) == true)
                {
                    ...
                    for (int i = 0; i < СписокПолейПути.Length; i++)
                        if (СписокПолейПути[i].Name == tsmi.Name)
                        {
                            ...
                            if (старыйНомерПути != данные.НомерПути)
                                Program.ЗаписьЛога("Действие оператора",
                                    "Изменение настроек поезда: " + данные.НомерПоезда + " " + данные.НазваниеПоезда +
                                    ": " + "Путь: " + старыйНомерПути + " -> " + данные.НомерПути + "; ",
                                    _authentificationService.CurrentUser);

                            данные.ТипСообщения = SoundRecordType.ДвижениеПоезда;
                            данные.НазванияТабло = данные.НомерПути != "0"
                                ? Binding2PathBehaviors.Select(beh => beh.GetDevicesName4Path(данные.НомерПути))
                                    .Where(str => str != null).ToArray()
                                : null;
                    ...
                    ToolStripMenuItem[] СписокНумерацииВагонов = new ToolStripMenuItem[]
                        {отсутсвуетToolStripMenuItem, сГоловыСоставаToolStripMenuItem, сХвостаСоставаToolStripMenuItem};
                    string[] СтроковыйСписокНумерацииВагонов = new string[]
                    {
                        "отсутсвуетToolStripMenuItem", "сГоловыСоставаToolStripMenuItem",
                        "сХвостаСоставаToolStripMenuItem"
                    };
                    if (СтроковыйСписокНумерацииВагонов.Contains(tsmi.Name))
                        for (int i = 0; i < СтроковыйСписокНумерацииВагонов.Length; i++)
                            if (СтроковыйСписокНумерацииВагонов[i] == tsmi.Name)
                            {
                                ...
                                if (СтараяНумерацияПоезда != данные.НумерацияПоезда)
                                    Program.ЗаписьЛога("Действие оператора",
                                        "Изменение настроек поезда: " + данные.НомерПоезда + " " +
                                        данные.НазваниеПоезда + ": " + "Нум.пути: " + СтараяНумерацияПоезда.ToString() +
                                        " -> " + данные.НумерацияПоезда.ToString() + "; ",
                                        _authentificationService.CurrentUser);
                                SoundRecords[КлючВыбранныйМеню] = данные;
                    ...
                    ToolStripMenuItem[] СписокКоличестваПовторов = new ToolStripMenuItem[]
                        {повтор1ToolStripMenuItem, повтор2ToolStripMenuItem, повтор3ToolStripMenuItem};
                    string[] СтроковыйСписокКоличестваПовторов = new string[]
                        {"повтор1ToolStripMenuItem", "повтор2ToolStripMenuItem", "повтор3ToolStripMenuItem"};
                    if (СтроковыйСписокКоличестваПовторов.Contains(tsmi.Name))
                        for (int i = 0; i < СтроковыйСписокКоличестваПовторов.Length; i++)
                            if (СтроковыйСписокКоличестваПовторов[i] == tsmi.Name)
                            {
                                ...
                                if (СтароеКоличествоПовторений != данные.КоличествоПовторений)
                                    Program.ЗаписьЛога("Действие оператора",
                                        "Изменение настроек поезда: " + данные.НомерПоезда + " " +
                                        данные.НазваниеПоезда + ": " + "Кол.повт.: " +
                                        СтароеКоличествоПовторений.ToString() + " -> " +
                                        данные.КоличествоПовторений.ToString() + "; ",
                                        _authentificationService.CurrentUser);
                                SoundRecords[КлючВыбранныйМеню] = данные;
                    ...
                    if (шаблоныОповещенияToolStripMenuItem1.DropDownItems.Contains(tsmi))
                    {
                        ...
                        if (ИндексШаблона >= 0 && ИндексШаблона < 10 &&
                            ИндексШаблона < данные.ActionTrainDynamiсList.Count)
                        {
        ...
        private void lVСобытия_ОбновитьСостояниеТаблицы()
        {
            ...
            foreach (var taskSound in TaskManager.Tasks)
            {
                ...
                else
                {
                    ...
                    {
                        case StateTask.Disabled:
                            if (lVСобытия.Items[номерСтроки].BackColor != Color.LightGray)
                                lVСобытия.Items[номерСтроки].BackColor = Color.LightGray;
                            break;

                        case StateTask.Enable:
                            if (lVСобытия.Items[номерСтроки].BackColor != Color.Chartreuse)
                                lVСобытия.Items[номерСтроки].BackColor = Color.Chartreuse;
                            break;
                        ...
                        case StateTask.Waiting:
                            ...
                            {
                                case MessageType.Статическое:
                                    if (lVСобытия.Items[номерСтроки].BackColor != Color.LightGreen)
                                        lVСобытия.Items[номерСтроки].BackColor = Color.LightGreen;
                                    break;
                                case MessageType.Динамическое:
                                    if (lVСобытия.Items[номерСтроки].BackColor != Color.White)
                                        lVСобытия.Items[номерСтроки].BackColor = Color.White;
                                    break;
                                case MessageType.ДинамическоеАварийное:
                                    if (lVСобытия.Items[номерСтроки].BackColor != Color.Orange)
                                        lVСобытия.Items[номерСтроки].BackColor = Color.Orange;
                                    break;
                            }
                            break;
                ...
                switch (subtaitle.MessageType)
                {
                    case MessageType.Статическое:
                        if (СтатическиеЗвуковыеСообщения.ContainsKey(subtaitle.Key))
                        {
                            ...
                            var currentStatSound =
                                StaticSoundForm.StaticSoundRecords.FirstOrDefault(sound =>
                                    sound.Name == statSound.НазваниеКомпозиции);
                    ...
                    case MessageType.Динамическое:
                        if (SoundRecords.ContainsKey(subtaitle.Key))
                        {
                            ...
                            var actionTrainDyn =
                                rec.ActionTrainDynamiсList.FirstOrDefault(atd => atd.Id == subtaitle.ParentId);
                            if (actionTrainDyn != null)
                            {
                                var textFragments =
                                    _soundReсordWorkerService.CalcTextFragment(ref rec, actionTrainDyn.ActionTrain);
                    ...
                    case MessageType.ДинамическоеАварийное:
                        ...
                        {
                            var rec = SoundRecords[subtaitle.Key];
                            var actionEmergencyDyn =
                                rec.EmergencyTrainDynamiсList.FirstOrDefault(atd => atd.Id == subtaitle.ParentId);
                            if (actionEmergencyDyn != null)
                            {
                                var textFragments =
                                    _soundReсordWorkerService.CalcTextFragment(ref rec, actionEmergencyDyn.ActionTrain);
                                rtb_subtaitles.ShowTextFragment(textFragments);
                            }
                        }
                        break;
                                ...
                                if (Key == Key2)
                                {
                                    ...
                                    for (int i = 0; i < lVСтатическиеСообщения.Items.Count; i++)
                                        if (lVСтатическиеСообщения.Items[i].SubItems[0].Text == Key)
                                            if (lVСтатическиеСообщения.Items[i].SubItems[1].Text !=
                                                Данные.НазваниеКомпозиции)
                                            {
                                                lVСтатическиеСообщения.Items[i].SubItems[1].Text =
                                                    Данные.НазваниеКомпозиции;
                                                break;
                                            }
                                }
        ...
        private SoundRecord ИзменениеДанныхВКарточке(SoundRecord старыеДанные, SoundRecord данные, string key)
        {
            ...
            {
                данные.НазванияТабло = (данные.НомерПути != "0" && !string.IsNullOrEmpty(данные.НомерПути))
                    ? MainWindowForm.Binding2PathBehaviors.Select(beh => beh.GetDevicesName4Path(данные.НомерПути))
                        .Where(str => str != null).ToArray()
                    : null;
            }
            ...
            if (старыеДанные.НазваниеПоезда != данные.НазваниеПоезда)
                сообщениеОбИзменениях +=
                    "Поезд: " + старыеДанные.НазваниеПоезда + " -> " + данные.НазваниеПоезда + "; ";
            if (старыеДанные.НомерПоезда != данные.НомерПоезда)
                сообщениеОбИзменениях += "№Поезда: " + старыеДанные.НомерПоезда + " -> " + данные.НомерПоезда + "; ";
            if (старыеДанные.НомерПути != данные.НомерПути)
                сообщениеОбИзменениях += "Путь: " + старыеДанные.НомерПути + " -> " + данные.НомерПути + "; ";
            if (старыеДанные.НумерацияПоезда != данные.НумерацияПоезда)
                сообщениеОбИзменениях += "Нум.пути: " + старыеДанные.НумерацияПоезда.ToString() + " -> " +
                                         данные.НумерацияПоезда.ToString() + "; ";
            if (старыеДанные.СменнаяНумерацияПоезда != данные.СменнаяНумерацияПоезда)
                сообщениеОбИзменениях += " сменная Нум.пути: " + старыеДанные.СменнаяНумерацияПоезда.ToString() +
                                         " -> " + данные.СменнаяНумерацияПоезда.ToString() + "; ";
            if (старыеДанные.СтанцияОтправления != данные.СтанцияОтправления)
                сообщениеОбИзменениях += "Ст.Отпр.: " + старыеДанные.СтанцияОтправления + " -> " +
                                         данные.СтанцияОтправления + "; ";
            if (старыеДанные.СтанцияНазначения != данные.СтанцияНазначения)
                сообщениеОбИзменениях += "Ст.Назн.: " + старыеДанные.СтанцияНазначения + " -> " +
                                         данные.СтанцияНазначения + "; ";
            if ((старыеДанные.БитыАктивностиПолей & 0x04) != 0x00)
                if (старыеДанные.ВремяПрибытия != данные.ВремяПрибытия)
                    сообщениеОбИзменениях += "Прибытие: " + старыеДанные.ВремяПрибытия.ToString("HH:mm") + " -> " +
                                             данные.ВремяПрибытия.ToString("HH:mm") + "; ";
            if ((старыеДанные.БитыАктивностиПолей & 0x10) != 0x00)
                if (старыеДанные.ВремяОтправления != данные.ВремяОтправления)
                    сообщениеОбИзменениях += "Отправление: " + старыеДанные.ВремяОтправления.ToString("HH:mm") +
                                             " -> " + данные.ВремяОтправления.ToString("HH:mm") + "; ";
            if (старыеДанные.Автомат != данные.Автомат)
                сообщениеОбИзменениях += "Режим работы измененн: " +
                                         (старыеДанные.Автомат ? "Автомат" : "Ручное") + " -> " +
                                         (данные.Автомат ? "Автомат" : "Ручное") + "; ";
            if (старыеДанные.ФиксированноеВремяПрибытия != данные.ФиксированноеВремяПрибытия)
                сообщениеОбИзменениях += "Фиксированное время ПРИБЫТИЯ измененно: " +
                                         ((старыеДанные.ФиксированноеВремяПрибытия == null)
                                             ? "--:--"
                                             : старыеДанные.ФиксированноеВремяПрибытия.Value.ToString("HH:mm")) +
                                         " -> " +
                                         ((данные.ФиксированноеВремяПрибытия == null)
                                             ? "--:--"
                                             : данные.ФиксированноеВремяПрибытия.Value.ToString("HH:mm")) + "; ";
            if (старыеДанные.ФиксированноеВремяОтправления != данные.ФиксированноеВремяОтправления)
                сообщениеОбИзменениях += "Фиксированное время ОТПРАВЛЕНИЯ измененно: " +
                                         ((старыеДанные.ФиксированноеВремяОтправления == null)
                                             ? "--:--"
                                             : старыеДанные.ФиксированноеВремяОтправления.Value.ToString("HH:mm")) +
                                         " -> " +
                                         ((данные.ФиксированноеВремяОтправления == null)
                                             ? "--:--"
                                             : данные.ФиксированноеВремяОтправления.Value.ToString("HH:mm")) + "; ";

            if (сообщениеОбИзменениях != "")
                Program.ЗаписьЛога("Действие оператора",
                    "Изменение настроек поезда: " + старыеДанные.НомерПоезда + " " + старыеДанные.НазваниеПоезда +
                    ": " + сообщениеОбИзменениях, _authentificationService.CurrentUser);

            return данные;
        }


        private void СохранениеИзмененийДанныхКарточкеБд(SoundRecord старыеДанные, SoundRecord данные,
            string источникИзменений = "Текущий пользователь")
        {
            var recChange = new SoundRecordChange
            {
                ...
                NewRec = данные,
                UserInfo =
                    $"{_authentificationService.CurrentUser.Login}  ({_authentificationService.CurrentUser.Role})",
